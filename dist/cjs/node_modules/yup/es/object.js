...
var sortFields = require('./util/sortFields.js');
var sortByKeyOrder = require('./util/sortByKeyOrder.js');
...
const defaultSort = sortByKeyOrder["default"]([]);
class ObjectSchema extends schema["default"] {
  constructor(spec) {
    super({
      type: 'object'
    });
  ...
  _validate(_value, opts = {}, callback) {
    ...
    super._validate(_value, opts, (err, value) => {
      if (err) {
        if (!ValidationError["default"].isError(err) || abortEarly) {
  ...
  concat(schema$1) {
    let next = super.concat(schema$1);
    ...
    for (let [field, schemaOrRef] of Object.entries(this.fields)) {
      ...
      } else if (target instanceof schema["default"] && schemaOrRef instanceof schema["default"]) {
  ...
  shape(additions, excludes = []) {
    ...
    next.fields = fields;
    next._sortErrors = sortByKeyOrder["default"](Object.keys(fields));
    ...
    next._nodes = sortFields["default"](fields, next._excludedEdges);
    return next;
  }
  ...
  unknown(allow = true, message = locale.object.noUnknown) {
