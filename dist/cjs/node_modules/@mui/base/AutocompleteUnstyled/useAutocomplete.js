...
function useAutocomplete(props) {
  ...
  const [inputValue, setInputValueState] = useControlled["default"]({
  ...
  const previousProps = usePreviousProps["default"]({
    filteredOptions,
    value
  });
  ...
  const focusTag = useEventCallback["default"](tagToFocus => {
    if (tagToFocus === -1) {
  ...
  const setHighlightedIndex = useEventCallback["default"](({
    event,
    index,
    reason = 'auto'
  }) => {
  ...
  const changeHighlightedIndex = useEventCallback["default"](({
    event,
    diff,
    direction = 'next',
    reason = 'auto'
  ...
  const handleListboxRef = useEventCallback["default"](node => {
    setRef["default"](listboxRef, node);
    if (!node) {
      return;
    }
  ...
  return {
    getRootProps: (other = {}) => _extends["default"]({
